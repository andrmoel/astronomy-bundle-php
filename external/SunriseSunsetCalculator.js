...
// was made, true is the entry is valid.
	...
	function isValidInput(f, index, latLongForm) 
	{
		...
		{
			alert("The algorithm used is not valid for years outside of the range -1000 to 3000.");
			return false;
		}
...
//***********************************************************************/
	...
	function calcSun(riseSetForm, latLongForm, index, index2, estimate_DST) // estimate_DST variable added by AWK
	{
		if(index2 != 0)
		{
...
//		if (estimate_DST) {window.alert("Warning:\nDaylight Saving Time\nmay require manual adjustment")};			// added by AWK
...
// recalculate SelectedIndex for DST YesNo - - added by AWK
if (estimate_DST == true) {index = any_date_latitude_DST(latitude, parseInt(riseSetForm["year"].value), indexRS+1, parseInt(riseSetForm["day"].value), 3)/-60};
cityLatLong.dayAns.selectedIndex = index;
...
//*********************************************************************/
			...
			if (!nosunrise)		// Sunrise was found
			{
				var riseTimeLST = riseTimeGMT - (60 * zone) + daySavings;	
					//	in minutes
//				var riseStr = timeStringShortAMPM(riseTimeLST, JD); ......................time format has been changed by AWK
				var riseStr = timeStringDate(riseTimeLST, JD);
...
//				var setStr = timeStringShortAMPM(setTimeLST, JD); ........................time format has been changed by AWK
				var setStr = timeStringDate(setTimeLST, JD);
				var utcSetStr = timeStringDate(setTimeGMT, JD);
				riseSetForm["sunset"].value = setStr;
				riseSetForm["utcsunset"].value = utcSetStr;
			}

			// Calculate the length of the day ...........................................added by AWK
			if (!nosunrise && !nosunset)
			{
				var LengthOfDayStr = timeString(setTimeLST - riseTimeLST);
				riseSetForm["daylength"].value = LengthOfDayStr;
			}	
...
// END of SUNrise & SUNset

// do next day - // added by AWK
function advance() {
	year = document.riseSetCalc.year.value;
	month = document.riseSetCalc.mos.selectedIndex;
	day = document.riseSetCalc.day.value;
		formUTCdate.setFullYear(year, month, day);

    formUTCdate.setDate(formUTCdate.getDate() + 1);

    var day = formUTCdate.getDate();
    var month = formUTCdate.getMonth();
    var year = formUTCdate.getFullYear();

	document.riseSetCalc.year.value = year;
	document.riseSetCalc.mos.selectedIndex = month;
	document.riseSetCalc.day.value = day;

	calcSun(riseSetCalc, cityLatLong, cityLatLong.dayAns.selectedIndex, cityLatLong.cities.selectedIndex, false);
}

// do previous day - // added by AWK
function backup() {
	year = document.riseSetCalc.year.value;
	month = document.riseSetCalc.mos.selectedIndex;
	day = document.riseSetCalc.day.value;
